(() => {
  ...
  function bind(){const input=$('#v41-files');if(!input||input.dataset.v69bound)return;input.dataset.v69bound='1';const prior=input.getAttribute('accept')||'';const add=['.pdf','.docx','.pptx'];input.setAttribute('accept',[...new Set(prior.split(',').map(x=>x.trim()).filter(Boolean).concat(add))].join(','));input.addEventListener('change',()=>processInput(input))}
