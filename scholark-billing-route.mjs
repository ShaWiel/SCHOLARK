...
console.log('[SCHOLARK] Paddle billing route ready · '+ENV+' · '+(configured()?'configured':'awaiting credentials')+' · key '+keyKind+' · token '+tokenKind+' · key-shape '+(keyShape.masked?'masked':keyShape.knownPrefix?'valid-prefix':'unknown-prefix'));
...
async function verifyCatalogPrice(plan,id,expectedAmount){
  const r=await paddle('/prices/'+encodeURIComponent(id)+'?include=product',{method:'GET'}),d=await r.json().catch(()=>({})),p=d?.data||{},product=p?.product||{};
  ...
  return {ok:active&&monthly&&trial7&&amount===String(expectedAmount)&&currency==='USD'&&!legacyBrand,plan,id:p?.id||id,productId:product?.id||p?.product_id||'',priceName,productName,productDescription,legacyBrand,amount,currency,monthly,trial7,active,requiresPaymentMethod:p?.trial_period?.requires_payment_method!==false};
}
...
http.Server.prototype.emit=function(type,...args){if(type!=='request')return previousEmit.call(this,type,...args);const [req,res]=args;let url;try{url=new URL(req.url||'/','http://localhost')}catch{return previousEmit.call(this,type,...args)}
  if(req.method==='GET'&&url.pathname==='/api/billing/config'){json(res,200,{ok:true,provider:'paddle',environment:ENV,configured:configured(),clientToken:configured()?CLIENT_TOKEN:'',priceIds:configured()?{plus:PRICES.plus,pro:PRICES.pro}:{},plans:{plus:14.99,pro:19.99},currency:'USD'});return true}
