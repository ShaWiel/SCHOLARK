...
const checkoutConfigured=()=>!!(SB&&PUB&&SERVICE&&CLIENT_TOKEN&&API_KEY&&/^pri_[a-z\d]{26}$/.test(PRICES.plus)&&/^pri_[a-z\d]{26}$/.test(PRICES.pro)&&credentialEnvironmentMatches());
const webhookConfigured=()=>checkoutConfigured()&&!!WEBHOOK_SECRET;
const configured=checkoutConfigured;
const keyShape={knownPrefix:keyKind!=='unknown',masked:/\*{2,}/.test(API_KEY),length:API_KEY.length};
console.log('[SCHOLARK] Paddle billing route ready · '+ENV+' · checkout '+(checkoutConfigured()?'configured':'awaiting credentials')+' · webhook '+(webhookConfigured()?'configured':'repairable')+' · key '+keyKind+' · token '+tokenKind+' · key-shape '+(keyShape.masked?'masked':keyShape.knownPrefix?'valid-prefix':'unknown-prefix'));
...
async function verifyCatalogPrice(plan,id,expectedAmount){
  ...
  if(!r.ok&&(r.status===401||r.status===403)){
    brandAudit='product_read_forbidden';
    r=await paddle('/prices/'+encodeURIComponent(id),{method:'GET'});d=await r.json().catch(()=>({}));
  }
  const p=d?.data||{},product=p?.product||{};
  if(!r.ok)return {ok:false,http:r.status,reason:d?.error?.code||d?.error?.type||'price_read_failed',brandAudit};
...
async function billingSelftest(){
  if(!checkoutConfigured()){catalogHealth={checked:true,ok:false,environment:ENV,reason:'checkout_credentials_or_prices_missing'};console.warn('[SCHOLARK] Paddle catalog self-test SKIP · checkout credentials incomplete');return catalogHealth}
  try{
...
async function handleSubscriptionEvent(event){const data=event.data||{},subId=String(data.id||''),existing=await getExistingSubscription(subId);const custom=data.custom_data||{};const userId=String(custom.scholark_user_id||existing?.user_id||'');if(!/^[0-9a-f-]{36}$/i.test(userId))return;const item=Array.isArray(data.items)?data.items.find(x=>x?.price?.id)||data.items[0]:null,price=String(item?.price?.id||existing?.price_id||''),plan=planFromPrice(price)||existing?.plan||'free';const status=String(data.status||'inactive'),active=['active','trialing','past_due'].includes(status),period=data.current_billing_period||{},cycleAdvanced=!!(existing&&['active','trialing'].includes(status)&&period.starts_at&&existing.current_period_start&&String(period.starts_at)!==String(existing.current_period_start)),changed=!existing||existing.plan!==plan||(!['active','trialing','past_due'].includes(existing.status)&&active)||(['active','trialing','past_due'].includes(existing.status)&&!active)||cycleAdvanced;const fresh=await recordEvent(event,userId,subId);if(!fresh)return;const cancel=String(data.scheduled_change?.action||'')==='cancel';const row={user_id:userId,provider:'paddle',paddle_customer_id:data.customer_id||existing?.paddle_customer_id||null,paddle_subscription_id:subId||existing?.paddle_subscription_id||null,paddle_transaction_id:data.transaction_id||existing?.paddle_transaction_id||null,plan,status:['trialing','active','past_due','paused','canceled'].includes(status)?status:'inactive',price_id:price||null,currency_code:data.currency_code||existing?.currency_code||null,current_period_start:period.starts_at||null,current_period_end:period.ends_at||null,cancel_at_period_end:cancel,last_event_id:event.event_id,updated_at:new Date().toISOString()};const r=await sb('/rest/v1/billing_subscriptions?on_conflict=user_id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(row)});if(!r.ok)throw new Error('subscription_sync_failed');await syncWallet(userId,plan,active,changed)}
async function finalizeTransaction(user,transactionId){
  if(!/^txn_[a-z\d]{26}$/.test(String(transactionId||'')))return {ok:false,http:400,code:'INVALID_TRANSACTION'};
  let t=null,lastStatus='';
  for(const wait of [0,300,700,1200]){
    if(wait)await new Promise(r=>setTimeout(r,wait));
    const pr=await paddle('/transactions/'+encodeURIComponent(transactionId),{method:'GET'}),pd=await pr.json().catch(()=>({}));
    if(!pr.ok)return {ok:false,http:502,code:'PADDLE_TRANSACTION_VERIFY_FAILED',error:pd?.error?.detail||pd?.error?.type||'Could not verify transaction.'};
    t=pd?.data||null;lastStatus=String(t?.status||'');
    if(['completed','paid','billed'].includes(lastStatus))break;
  }
  if(!t)return {ok:false,http:502,code:'PADDLE_TRANSACTION_VERIFY_FAILED'};
  const custom=t.custom_data||{},owner=String(custom.scholark_user_id||'');
  if(owner!==user.id)return {ok:false,http:403,code:'TRANSACTION_OWNER_MISMATCH'};
  const item=Array.isArray(t.items)?t.items.find(x=>x?.price?.id===PRICES.plus||x?.price?.id===PRICES.pro)||t.items[0]:null;
  const price=String(item?.price?.id||''),plan=planFromPrice(price)||String(custom.scholark_plan||'').toLowerCase();
  if(!['plus','pro'].includes(plan)||PRICES[plan]!==price)return {ok:false,http:400,code:'TRANSACTION_PLAN_MISMATCH'};
  if(!['completed','paid','billed'].includes(lastStatus))return {ok:false,http:202,code:'TRANSACTION_PROCESSING',pending:true,status:lastStatus};
  const existing=await getExistingSubscription(String(t.subscription_id||''),user.id),changed=!existing||existing.plan!==plan||!['active','trialing','past_due'].includes(existing.status);
  const row={user_id:user.id,provider:'paddle',paddle_customer_id:t.customer_id||existing?.paddle_customer_id||null,paddle_subscription_id:t.subscription_id||existing?.paddle_subscription_id||null,paddle_transaction_id:t.id,plan,status:'trialing',price_id:price,currency_code:t.currency_code||existing?.currency_code||'USD',current_period_start:existing?.current_period_start||null,current_period_end:existing?.current_period_end||null,cancel_at_period_end:false,last_event_id:'checkout:'+t.id,updated_at:new Date().toISOString()};
  const sr=await sb('/rest/v1/billing_subscriptions?on_conflict=user_id',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=minimal'},body:JSON.stringify(row)});
  if(!sr.ok)return {ok:false,http:500,code:'SUBSCRIPTION_PROVISION_FAILED'};
  await syncWallet(user.id,plan,true,changed);
  return {ok:true,http:200,plan,transactionId:t.id,subscriptionId:t.subscription_id||null,wallet:await billingStatus(user)};
}
...
http.Server.prototype.emit=function(type,...args){if(type!=='request')return previousEmit.call(this,type,...args);const [req,res]=args;let url;try{url=new URL(req.url||'/','http://localhost')}catch{return previousEmit.call(this,type,...args)}
  if(req.method==='GET'&&url.pathname==='/api/billing/config'){json(res,200,{ok:true,provider:'paddle',environment:ENV,configured:checkoutConfigured(),webhookConfigured:webhookConfigured(),clientToken:checkoutConfigured()?CLIENT_TOKEN:'',priceIds:checkoutConfigured()?{plus:PRICES.plus,pro:PRICES.pro}:{},plans:{plus:14.99,pro:19.99},currency:'USD'});return true}
  if(req.method==='GET'&&url.pathname==='/api/billing/health'){json(res,200,{ok:true,provider:'paddle',environment:ENV,configured:checkoutConfigured(),checkoutConfigured:checkoutConfigured(),webhookConfigured:webhookConfigured(),prices:{plus:!!PRICES.plus,pro:!!PRICES.pro},webhookSecret:!!WEBHOOK_SECRET,apiKey:!!API_KEY,clientToken:!!CLIENT_TOKEN,credentialEnvironmentMatches:credentialEnvironmentMatches(),productionReady:ENV==='production'&&checkoutConfigured()&&webhookConfigured()&&catalogHealth?.ok===true,catalog:catalogHealth});return true}
