...
ok(schoolStrict.includes("FHR Institute for Higher Education',exact:['hbo']")&&schoolStrict.includes('FHR Institute for Social Studies')&&schoolStrict.includes('bachelor- en masteropleidingen'),'FHR higher-education mapping is incomplete or incorrectly treated as a university');
...
ok(learningRoute.includes("mode==='general_ai'")&&learningRoute.includes("You are ARKI, the general-purpose AI assistant inside SCHOLARK")&&learningRoute.includes("history:Array.isArray(p.history)")&&learningRoute.includes("'tutor','general_ai','flashcards','exam'"),'General-purpose AI backend mode is incomplete');
ok(generalAi.includes("version:'20260920-r175'")&&generalAi.includes("mode:'general_ai'")&&generalAi.includes("Deep answer")&&generalAi.includes("Ctrl/Cmd + Enter to send"),'General ARKI chat UI is incomplete');
...
ok(workspaceShell.includes("['ai','✺','ARKI']")&&workspaceShell.includes("card('ai','✺','ARKI'")&&workspaceShell.includes("window.__SCHOLARK_V107_GENERAL_AI__"),'ARKI is missing from workspace navigation/dashboard routing');
...
ok(workspaceShell.includes('v51-levels-suriname')&&workspaceShell.includes('overflow-x:auto')&&workspaceShell.includes("host.scrollBy({left:")&&workspaceShell.includes('function wireLevelScroll()'),'Suriname dashboard level strip is not horizontally scrollable');
...
ok(runtime.includes("const HOME = ['scholark-v109-home-owner.js'")&&runtime.includes("'scholark-v55-home-topbar-workspace-entry.js','scholark-v99-home-foundation.js'")&&!workspaceBlock.includes('scholark-v109-home-owner.js'),'Home-only modules are not isolated from workspace boot');
...
ok(i18n.includes("LANGS.length===74")&&i18n.includes("dynamicLocales.length===67")&&i18n.includes("['ar','he','ur','fa','ps']"),'74-language catalog / RTL contract is incomplete');
...
ok(i18n.includes("dynamic=!STATIC_CORE_LANGS.has(target)")&&i18n.includes("home?collectDom(340):[...CORE,...collectDom(480)]")&&i18n.includes("scheduleLanguageCompletion(target,epoch)"),'Dynamic first-use translation priming is incomplete');
...
ok(i18n.includes("classList.add('scholark-home-language-adapting')")&&i18n.includes('freezeHomeSurface()')&&i18n.includes("dataset.scholarkI18nReady=target")&&i18n.includes('releaseHomeSurface()'),'Homepage language adaptation is not atomic');
...
ok(countryEducation.includes('STATIC_UI_LANGS')&&countryEducation.includes('Intl.DisplayNames')&&countryEducation.includes("window.__SCHOLARK_I18N__?.apply?.(document)"),'Country/education UI is not compatible with adaptive languages');
...
ok(runtime.includes('studio:[]')&&runtime.includes("planner:['scholark-v52-workspace-qa.js']")&&runtime.includes("project:['scholark-v64-projects.js'")&&runtime.includes("ai:['scholark-v107-general-ai.js']"),'Lazy feature routing is incomplete after workspace boot optimization');
...
ok(i18n.includes("CACHE_VERSION='v8-global74-resilient'")&&i18n.includes("'v5-global37','v4-seven-ui','v3-seven-ui'"),'global-74 i18n cache isolation/recovery is missing');
...
ok(homeFoundation.includes('scholark_v90_i18n_v8-global74-resilient_')&&!homeFoundation.includes("saved[source]=rows[lc]||source"),'Home foundation does not preserve dynamic-language caches correctly');
...
ok(studyAhead.includes('data-v83="goal"')&&studyAhead.includes('api?.data?.planner?.()')&&studyAhead.includes('api?.data?.mastery?.()')&&studyAhead.includes('api?.data?.goals?.()'),'Study Ahead is not connected to local Planner/Mastery/Goals');
