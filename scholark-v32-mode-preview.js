(() => {
  ...
  function healthy(host=$('.v29-preview'),mode=currentMode()){
    ...
    return !!shell&&!!pane&&shell.dataset.v32Mode===mode&&pane.children.length>0&&getComputedStyle(shell).display!=='none';
  }
  function render(force=false){
    ...
    lastMode=mode;
    requestAnimationFrame(()=>{if(!healthy(host,mode)){lastMode='';host.innerHTML='';requestAnimationFrame(()=>render(true))}});
  }
  ...
  addEventListener('scholark-language-ready',()=>setTimeout(rerender,70));
