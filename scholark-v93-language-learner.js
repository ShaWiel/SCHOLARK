(() => {
  ...
  function saveHistory(row){let h=history();h=[row,...h.filter(x=>x.id!==row.id)].slice(0,20);try{localStorage.setItem(HISTORY,JSON.stringify(h))}catch{}}
  ...
  function renderHistory(){
    const host=$('#v93-history');if(!host)return;const h=history();host.innerHTML=h.length?h.map((x,i)=>'<button type="button" data-v93-old="'+i+'"><b>'+esc(langName(x.targetCode))+' · '+esc(x.level||'A1')+'</b><span>'+esc(x.topic||x.result?.title||'Language lesson')+' · '+new Date(x.at||Date.now()).toLocaleDateString()+'</span></button>').join(''):'<div class="v93-chip">Your generated lessons will appear here.</div>';
