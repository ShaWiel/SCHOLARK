(() => {
  ...
  const css=document.createElement('style');css.id='scholark-v83-style';css.textContent=`
    ...
    .v83-status{font:750 8px/1.4 Inter;color:#6558c8;margin-top:8px}.v83-saved{margin:12px 0;display:grid;gap:7px}.v83-saved-card{border:1px solid rgba(23,25,31,.09);background:#fff;border-radius:14px;padding:12px}.v83-saved-card b{font:900 10px Inter}.v83-saved-card span{display:block;margin-top:4px;color:#77717e;font:650 8px/1.35 Inter}
  ...
  function renderSaved(){
    ...
    host.innerHTML=saved.length?'<div class="v52-kicker">SAVED STUDY AHEAD TRACKS</div><div class="v83-saved">'+saved.map(x=>'<div class="v83-saved-card"><b>'+esc(x.field)+(x.target_school?' · '+esc(x.target_school):'')+'</b><span>'+esc(x.country||'')+(x.updated_at?' · '+esc(new Date(x.updated_at).toLocaleDateString()):'')+'</span></div>').join('')+'</div>':'';
  }
  ...
  function decorate(statusText=''){
    ...
    let bar=$('#v83-actions',out);if(!bar){bar=document.createElement('div');bar.id='v83-actions';bar.innerHTML='<div class="v83-actions"><button type="button" data-v83="planner">Add roadmap to Planner</button><button type="button" class="alt" data-v83="mastery">Add key subjects to Mastery</button><button type="button" class="alt" data-v83="refresh">Save / refresh track</button></div><div class="v83-status"></div>';out.appendChild(bar);$$('[data-v83]',bar).forEach(b=>b.onclick=()=>action(b.dataset.v83,b))}
