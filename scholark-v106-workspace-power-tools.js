(() => {
  ...
  function renderFocus(){
    ...
    $('#v106-focus-custom').onchange=()=>{const n=Math.max(5,Math.min(240,Number($('#v106-focus-custom').value)||25)),host=$('#v106-focus-duration');host.dataset.value=String(n);$('[data-min]',host).forEach(x=>x.classList.remove('active'));const cur=focusState();if(!cur.running){cur.duration=n;cur.remaining=0;saveFocus(cur);syncFocusView()}};
