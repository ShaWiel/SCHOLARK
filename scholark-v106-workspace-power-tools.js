(() => {
  ...
  function openTutor(prompt){
    try{sessionStorage.setItem('scholark_v106_pending_tutor',prompt)}catch{}
  ...
  function renderFocus(){
    ...
    root.innerHTML=`<div class="v106-kicker">SCHOLARK · FOCUS SESSIONS</div><h1>Turn a plan into focused work.</h1><p>Run distraction-light study blocks, connect them to Planner tasks and keep a simple record of how much focused work you actually complete.</p><div class="v106-stat-grid"><div class="v106-stat"><b>${minutes}</b><span>Focus minutes today</span></div><div class="v106-stat"><b>${todayRows.length}</b><span>Sessions today</span></div><div class="v106-stat"><b>${week.length}</b><span>Sessions in 7 days</span></div><div class="v106-stat"><b>${hist.reduce((n,z)=>n+(Number(z.minutes)||0),0)}</b><span>Total focus minutes</span></div></div><div class="v106-focus-wrap"><div class="v106-timer"><div><small id="v106-focus-status">READY</small><time id="v106-focus-time">${fmt(focusRemaining(x))}</time><div style="font:750 9px Inter;color:#cbc7d4;margin-top:9px">${esc(x.task||'Choose one task and start')}</div></div></div><div class="v106-card"><h2>Session setup</h2><div class="v106-form"><input id="v106-focus-task" placeholder="What will you focus on?" value="${esc(x.task)}"><div class="v106-duration" id="v106-focus-duration" data-value="${x.duration}">${[25,45,60,90].map(n=>`<button type="button" data-min="${n}" class="${n===x.duration?'active':''}">${n} min</button>`).join('')}</div><select id="v106-focus-link"><option value="">No Planner task linked</option>${planner.map(p=>`<option value="${esc(p.id)}" ${p.id===x.linkedPlannerId?'selected':''}>${esc(p.text)}</option>`).join('')}</select><label class="v106-check"><input id="v106-focus-auto" type="checkbox" ${x.autoComplete?'checked':''}> Mark the linked Planner task complete when this focus session finishes.</label><div class="v106-row"><button class="v106-btn lime" id="v106-focus-main">${x.running?'Pause':focusRemaining(x)<x.duration*60000?'Resume':'Start focus'}</button><button class="v106-btn alt" id="v106-focus-reset">Reset</button></div></div></div></div><div class="v106-section-title">Recent focus sessions</div><div class="v106-list">${hist.slice(-8).reverse().map(z=>`<div class="v106-item"><b>${esc(z.task||'Focus session')}</b><div class="v106-meta"><span class="v106-badge">${z.minutes} min</span><span class="v106-badge">${esc(z.date||'')}</span></div></div>`).join('')||'<div class="v106-empty">No completed focus sessions yet.</div>'}</div>`;
    $$('#v106-focus-duration [data-min]').forEach(b=>b.onclick=()=>{const host=$('#v106-focus-duration');host.dataset.value=b.dataset.min;$$('[data-min]',host).forEach(x=>x.classList.toggle('active',x===b));const cur=focusState();if(!cur.running){cur.duration=Number(b.dataset.min);cur.remaining=0;saveFocus(cur);syncFocusView()}});
  ...
  function renderFlashcards(){
    ...
    $('#v106-study-start').onclick=()=>startCardReview($('#v106-deck-filter').value);
    drawList();
  }
  function startCardReview(deck){
  ...
  function assignmentTutor(a){
    openTutor('Help me plan and complete this '+a.type+'. Title: '+a.title+'. Subject: '+(a.subject||'not specified')+'. Due date: '+(a.dueDate||'not specified')+'. Requirements: '+(a.instructions||'No extra requirements provided')+'. First explain the task, then give me a strong step-by-step approach without doing dishonest work for me.');
  }
  function renderAssignments(){
    ...
    root.innerHTML=`<div class="v106-kicker">SCHOLARK · ASSIGNMENTS</div><h1>Turn assignments into a workable plan.</h1><p>Track briefs and deadlines, break an assignment into Planner steps, monitor progress and send the full context to AI Tutor when you need guidance.</p><div class="v106-stat-grid"><div class="v106-stat"><b>${active.length}</b><span>Active assignments</span></div><div class="v106-stat"><b>${soon.length}</b><span>Due within 7 days</span></div><div class="v106-stat"><b>${overdue.length}</b><span>Overdue</span></div><div class="v106-stat"><b>${all.filter(a=>a.status==='complete').length}</b><span>Completed</span></div></div><div class="v106-card"><h2>Add an assignment</h2><div class="v106-form"><div class="v106-row3"><input id="v106-a-title" placeholder="Assignment / project title"><input id="v106-a-subject" placeholder="Subject / course"><select id="v106-a-type"><option value="assignment">Assignment</option><option value="project">Project</option><option value="essay">Essay / report</option><option value="presentation">Presentation</option><option value="exam">Exam / test</option></select></div><div class="v106-row"><input id="v106-a-due" type="date"><input id="v106-a-progress" type="number" min="0" max="100" value="0" placeholder="Progress %"></div><textarea id="v106-a-info" placeholder="Requirements, rubric, instructions, topics or constraints"></textarea><button class="v106-btn" id="v106-a-add">Add assignment</button></div></div><div class="v106-section-title">Assignments</div><div class="v106-list" id="v106-a-list"></div>`;
    const draw=()=>{
      const a=assignments().sort((x,y)=>(x.status==='complete')-(y.status==='complete')||String(x.dueDate||'9999').localeCompare(String(y.dueDate||'9999')));saveAssignments(a);
      $('#v106-a-list').innerHTML=a.length?a.map(x=>{const d=daysUntil(x.dueDate),dueLabel=d==null?'No due date':d<0?Math.abs(d)+' day'+(Math.abs(d)===1?'':'s')+' overdue':d===0?'Due today':d+' day'+(d===1?'':'s')+' left';return `<div class="v106-item"><div class="v106-assignment-title"><div><strong>${esc(x.title)}</strong><div class="v106-meta"><span class="v106-badge">${esc(x.type)}</span>${x.subject?'<span class="v106-badge">'+esc(x.subject)+'</span>':''}<span class="v106-badge ${d!=null&&d<0?'warn':d!=null&&d<=3?'warn':'ok'}">${esc(dueLabel)}</span></div></div><span class="v106-badge ${x.status==='complete'?'ok':''}">${x.status==='complete'?'Complete':'Active'}</span></div>${x.instructions?'<p style="margin:7px 0 0">'+esc(x.instructions)+'</p>':''}<div class="v106-progress-row"><div class="v106-progress"><i style="width:${x.progress}%"></i></div><b style="font:900 8px Inter">${x.progress}%</b></div><div class="v106-inline" style="margin-top:8px"><input type="range" min="0" max="100" step="5" value="${x.progress}" data-a-progress="${esc(x.id)}"><span style="font:800 7px Inter;color:#777">Update progress</span></div><div class="v106-actions"><button class="primary" data-a-plan="${esc(x.id)}">Break into Planner</button><button data-a-tutor="${esc(x.id)}">Open in AI Tutor</button><button data-a-complete="${esc(x.id)}">${x.status==='complete'?'Reopen':'Complete'}</button><button data-a-del="${esc(x.id)}">Delete</button></div></div>`}).join(''):'<div class="v106-empty">No assignments yet. Add one above and SCHOLARK can connect it to Planner and AI Tutor.</div>';
    ...
    $('#v106-a-add').onclick=()=>{const title=$('#v106-a-title').value.trim();if(!title)return $('#v106-a-title').focus();const a=assignments();a.push({id:uid('assign'),title,subject:$('#v106-a-subject').value.trim(),type:$('#v106-a-type').value,dueDate:$('#v106-a-due').value,instructions:$('#v106-a-info').value.trim(),progress:Math.max(0,Math.min(100,Number($('#v106-a-progress').value)||0)),status:'active',createdAt:nowIso()});saveAssignments(a);renderAssignments()};draw();
  }
  ...
  window.__SCHOLARK_V106_POWER__={
    ...
    version:'20260918-workspace-power-v1'
  };
})();