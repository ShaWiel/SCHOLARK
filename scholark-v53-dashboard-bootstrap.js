(() => {
  ...
  const isWorkspace=()=>/dashboard|studio|tutor|education|planner|progress|goal|project|schools|study|book|presentation|webpage|document|report|graphic|social/.test(hash());
  ...
  style.textContent=`
    ...
    .v53-main{margin-left:258px;min-height:100vh;padding:34px;box-sizing:border-box}.v53-shell{max-width:1400px;margin:0 auto}.v53-kicker{font:900 8px Inter;letter-spacing:.14em;color:#6d5dfc}.v53-main h1{font:950 clamp(40px,5vw,62px)/.94 Inter;margin:10px 0 12px;letter-spacing:-.05em}.v53-main p{max-width:760px;color:#706c77;font:600 12px/1.55 Inter}.v53-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:24px}.v53-card{border:1px solid rgba(23,25,31,.1);background:#fff;border-radius:22px;padding:19px;min-height:160px;text-align:left;cursor:pointer;box-shadow:0 17px 50px rgba(31,27,63,.045)}.v53-card.primary{grid-column:span 2;background:linear-gradient(145deg,#17191f,#30275d);color:#fff}.v53-card h3{font:950 20px/1 Inter;margin:4px 0 8px}.v53-card p{font:600 10px/1.5 Inter;margin:0;color:#77727d}.v53-card.primary p{color:#cac6d2}.v53-card b{display:inline-block;margin-top:18px;color:#6d5dfc;font:900 8.5px Inter}.v53-card.primary b{color:#c9ff6a}
    @media(max-width:900px){.v53-grid{grid-template-columns:repeat(2,1fr)}.v53-card.primary{grid-column:span 2}}
    @media(max-width:640px){.v53-side{width:74px}.v53-brand{font-size:0}.v53-brand:before{content:'S';font:950 18px Inter;color:#c9ff6a}.v53-brand small,.v53-nav button{font-size:0}.v53-nav button:before{font-size:13px}.v53-main{margin-left:74px;padding:22px 14px}.v53-grid{grid-template-columns:1fr}.v53-card.primary{grid-column:auto}}
  `;
  ...
  function createEmergency(){
    ...
    root.innerHTML=`<aside class="v53-side"><div class="v53-brand">SCHOLARK<small>WORKSPACE</small></div><div class="v53-nav">
      <button class="active" data-v51-tool="dashboard">⌂ Dashboard</button><button data-v51-tool="studio">✦ Studio AI</button><button data-v51-tool="tutor">AI Tutor</button><button data-v51-tool="education">Education & Learning</button><button data-v51-tool="planner">Planner</button><button data-v51-tool="progress">Progress</button><button data-v51-tool="goal">Goals</button><button data-v51-tool="project">My Projects</button><button data-v51-tool="schools">Schools Near Me</button><button data-v51-tool="study">Study Ahead</button><button data-v51-tool="book">Book Studio</button>
    </div></aside><main class="v53-main"><div class="v53-shell"><div class="v53-kicker">SCHOLARK WORKSPACE</div><h1>Your learning & creation workspace.</h1><p>This fail-safe only appears if the primary workspace shell failed to mount. The real V51 workspace remains the preferred interface.</p><div class="v53-grid">
      <button class="v53-card primary" data-v51-tool="studio"><h3>Studio AI</h3><p>Create presentations, webpages, documents, social content and graphics.</p><b>OPEN STUDIO →</b></button>
      <button class="v53-card" data-v51-tool="tutor"><h3>AI Tutor</h3><p>Learn and practice with adaptive AI support.</p><b>OPEN TUTOR →</b></button>
      <button class="v53-card" data-v51-tool="education"><h3>Education & Learning</h3><p>Diagnostics, learning paths and mastery support.</p><b>OPEN LEARNING →</b></button>
      <button class="v53-card" data-v51-tool="planner"><h3>Planner</h3><p>Plan study work, goals and deadlines.</p><b>OPEN PLANNER →</b></button>
      <button class="v53-card" data-v51-tool="progress"><h3>Progress</h3><p>Track what is improving and what needs focus.</p><b>VIEW PROGRESS →</b></button>
      <button class="v53-card" data-v51-tool="goal"><h3>Goals</h3><p>Set learning and school goals.</p><b>OPEN GOALS →</b></button>
    </div></div></main>`;
    document.body.appendChild(root);
    $$('[data-v51-tool]',root).forEach(btn=>btn.addEventListener('click',()=>{
      const id=btn.dataset.v51Tool;
  ...
  function ensurePrimaryDashboard(){
  ...
  function schoolBridge(){
    ...
    setTimeout(()=>{const b=$('[data-v50-school="1"]')||t;if(b&&b.dataset.v50School==='1')try{b.click()}catch{}},220);
  }

  function sync(){
    if(!isWorkspace()){document.body.classList.remove('v53-emergency');return;}
    if(isDashboard()){
      if(ensurePrimaryDashboard())return;
      setTimeout(()=>{
