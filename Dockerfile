...
RUN unzip /tmp/scholark.zip -d /app \
    ...
    && find /app -type f -name '*.html' -exec sh -c 'cp /tmp/scholark-v24-ui.js "$(dirname "$1")/scholark-v24-ui.js"; sed -i "s#</body>#<script src=\"scholark-v24-ui.js\"></script></body>#" "$1"' sh {} \; \
    && rm /tmp/scholark.zip /tmp/scholark_v23_patch.gz.b64 /tmp/scholark_v23_education.gz.b64 /tmp/scholark_v23.patch /tmp/scholark-v24-ui.js
