...
RUN unzip /tmp/scholark.zip -d /app \
    ...
    && find /app -type f -name '*.html' -exec sh -c 'dir=$(dirname "$1"); tags=""; for f in /tmp/scholark-v*.js; do base=$(basename "$f"); cp "$f" "$dir/$base"; tags="$tags<script defer src=\"$base?v=20260829-r106\"></script>"; done; sed -i "s#</body>#$tags</body>#" "$1"' sh {} \; \
