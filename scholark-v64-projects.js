(() => {
  ...
  function history(){try{return JSON.parse(localStorage.getItem('scholark_v45_history')||'[]')}catch{return[]}}
  function key(x){return x.deckId?'deck:'+x.deckId:x.artifactId?'artifact:'+x.artifactId:[x.mode||'',x.project||'',x.rawPrompt||x.prompt||''].join('|')}
  function dedupe(a=history()){const seen=new Set(),out=[];for(const x of a){const k=key(x);if(seen.has(k))continue;seen.add(k);out.push(x)}try{localStorage.setItem('scholark_v45_history',JSON.stringify(out.slice(0,40)))}catch{}return out.slice(0,40)}
  function host(){
    ...
    $$('#v51-sidebar [data-v51-tool]').forEach(b=>b.classList.toggle('active',b.dataset.v51Tool==='project'));history.replaceState(null,'',location.pathname+location.search+'#project');
  ...
  function removeItem(i){const a=dedupe(),x=a[i];if(!x)return;a.splice(i,1);try{localStorage.setItem('scholark_v45_history',JSON.stringify(a));if(x.artifactId)localStorage.removeItem('scholark_v58_artifact_'+x.artifactId);if(x.deckId)localStorage.removeItem('scholark_v57_deck_'+x.deckId)}catch{}render()}
