(() => {
  ...
  const css=document.createElement('style');css.id='scholark-v51-layout-style';css.textContent=`
    ...
    /* Collapsed means fully gone: override V47/V49 shell rules too. */
    body.v48-workspace.v49-sidebar-collapsed #v48-sidebar,
    body.v48-workspace.v49-sidebar-collapsed #v48-sidebar.v41-sidebar-shell,
    body.v48-workspace.v49-sidebar-collapsed #v48-sidebar.v40-sidebar-shell{
      ...
      box-shadow:none!important;overflow:hidden!important;display:block!important;
    }
    ...
    body.v49-sidebar-collapsed .v49-native-panel{left:0!important;margin-left:0!important;width:auto!important;max-width:none!important}
    ...
    body.v51-pro-page #v25-book{
      position:fixed!important;left:var(--v51-left,258px)!important;top:0!important;right:0!important;bottom:0!important;
    ...
    body.v49-sidebar-collapsed.v51-pro-page #v25-book{left:0!important}
  ...
  function toggleVisibility(){const b=$('#v49-sidebar-toggle');if(b){const active=workspace()&&document.body.classList.contains('v48-workspace');b.hidden=!active;b.style.setProperty('display',active?'grid':'none','important');}}
  ...
  function ensureStudyPage(){
    ...
    if(!$('.v25-content',d)?.children.length){const trigger=$('[data-tool="study"]');try{trigger?.click()}catch{}}
    return true;
  }
  function ensureBookPage(){
    const d=$('#v25-book');if(!d)return false;
    d.classList.add('open');
    if(!$('.v25-content',d)?.children.length){const trigger=$('[data-tool="book"]');try{trigger?.click()}catch{}}
    return true;
  }
  ...
  function syncProPage(){
    ...
    $('#v48-dashboard')?.setAttribute('hidden','');$('#v29-home-layer')?.setAttribute('hidden','');
    if(h.includes('schools'))ensureSchoolsPage();else if(h.includes('study'))ensureStudyPage();else if(h.includes('book'))ensureBookPage();
  }

  /* Study Ahead and Book Studio are promoted from modal handlers into docked pages. Schools keeps V50's own click/search handler, then V51 docks it. */
  document.addEventListener('click',e=>{
    const b=e.target.closest('#v48-sidebar [data-v48-tool],#v48-dashboard [data-v48-tool]');if(!b)return;
    const id=b.dataset.v48Tool;if(!['study','book'].includes(id))return;
    e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
    history.pushState(null,'',location.pathname+location.search+'#'+id);window.dispatchEvent(new HashChangeEvent('hashchange'));setTimeout(syncProPage,0);
  },true);

  /* V49 still owns the stored collapse preference; V51 owns the full-screen geometry. */
