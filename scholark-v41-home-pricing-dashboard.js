(() => {
  ...
  style.textContent=`
    #sv24-home,#sv24-launch,#v34-dashboard-entry,#v36-shell-home,#v40-workspace-home,#v26-sidebar-toggle,#v37-home-pricing,#v39-home-pricing,#v40-home-pricing,#v37-pro-suite,#v41-dashboard-entry{display:none!important}
    ...
    @media(max-width:1120px){.v41-price-grid{grid-template-columns:1fr}.v41-plan{min-height:auto}#v41-home-pricing{padding:0 16px}.v41-plan.v41-focus{transform:none}#sv24-overlay .sv24-modes{grid-template-columns:repeat(3,minmax(0,1fr))!important}}
  `;
  ...
  function openTool(tool){if(tool==='studio'||tool==='book'){window.__SCHOLARK_WORKSPACE__?.openTool?.(tool);if(!window.__SCHOLARK_WORKSPACE__?.openTool)location.hash=tool;return}window.__SCHOLARK_WORKSPACE__?.openTool?.(tool);if(!window.__SCHOLARK_WORKSPACE__?.openTool)location.hash=tool;}
  function ensureProTools(){const inner=findSidebarInner();if(!inner)return;let box=$('#v41-sidebar-pro');if(!box){box=document.createElement('div');box.id='v41-sidebar-pro';box.innerHTML='<div class="title">PRO TOOLS</div><button class="v41-side" data-v41="schools">⌖ <span>Schools Near Me</span><em>PRO</em></button><button class="v41-side" data-v41="study">↗ <span>Study Ahead</span><em>PRO</em></button>';inner.appendChild(box);box.querySelector('[data-v41="schools"]').onclick=()=>openTool('schools');box.querySelector('[data-v41="study"]').onclick=()=>openTool('study');}}
  ...
  function ensureStudio(){
    if(window.__SCHOLARK_FEATURE_FLAGS__?.studio===false){$('#sv24-overlay')?.classList.remove('open');$('#v41-studio-note')?.remove();return}
  ...
  function priceMarkup(){return `<div class="v41-price-head"><small>SCHOLARK PLANS</small><h2>Choose how much advantage you want.</h2><p>Start free, move to Plus for more AI and research, or choose Pro when you want SCHOLARK to connect your planning, progress and future-study journey.</p></div><div class="v41-price-grid">
    <article class="v41-plan"><div class="v41-kicker">FREE</div><h3>SCHOLARK Free</h3><div class="v41-desc">For everyday learning, practice, planning and AI support.</div><div class="v41-price">$0 <small>/ month</small></div><div class="v41-trial">No payment method required.</div><ul>
      <li>ARKI general AI + AI Tutor</li><li>Education & Learning: diagnostics, Mastery & Spaced Review</li><li>Planner, Focus Sessions, Goals & Progress</li><li>Flashcards + Assignments</li><li>Files & Notes analysis</li><li>Language Learner</li><li>My Projects workspace</li><li>30 included AI credits/month</li>
    </ul><button data-plan="free">Start free</button></article>
    <article class="v41-plan plus"><div class="v41-kicker">PLUS</div><h3>SCHOLARK Plus</h3><div class="v41-desc">For serious learners who need more AI, research and project capacity.</div><div class="v41-price">$14.99 <small>/ month</small></div><div class="v41-trial">7 days free, then $14.99/month. Cancel anytime.</div><ul>
      <li>Everything in Free</li><li>Research + citations and web sources</li><li>Cloud projects + version history</li><li>Advanced Files & Notes: Ask Files, worksheets and study tools</li><li>More ARKI, AI Tutor and learning workflow usage</li><li>750 included AI credits/month</li><li>Higher fair-use limits</li>
    </ul><button data-plan="plus">Start Plus free trial</button></article>
    <article class="v41-plan pro"><span class="v41-most">MOST POPULAR</span><div class="v41-kicker">PRO</div><h3>SCHOLARK Pro</h3><div class="v41-desc">For ambitious learners who want SCHOLARK to plan, analyze and guide their full learning journey.</div><div class="v41-price">$19.99 <small>/ month</small></div><div class="v41-trial">7 days free, then $19.99/month. Cancel anytime.</div><ul>
      <li>Everything in Plus</li><li>Smart Study Planning across Study Ahead, Planner, Goals & Mastery</li><li>Advanced Progress Insights: focus, flashcards, assignments & planner completion</li><li>Schools Near Me: country- and level-aware school discovery</li><li>Study Ahead: AI-powered future-study roadmap</li><li>Cross-tool ARKI support for files, lessons, roadmaps & school comparisons</li><li>2,500 included AI credits/month</li><li>Highest fair-use limits</li>
  ...
  function ensurePricing(){
    ...
    let p=$('#v41-home-pricing');if(!p){p=document.createElement('section');p.id='v41-home-pricing';p.innerHTML=priceMarkup();const shell=$('.v29-shell',layer)||layer;const final=$('.v29-final,.v29-final-cta,[class*="final-cta"]',shell);final?shell.insertBefore(p,final):shell.appendChild(p);window.__SCHOLARK_I18N__?.apply?.(p);setTimeout(()=>window.__SCHOLARK_I18N__?.translateMissing?.(),60);$$('[data-plan]',p).forEach(b=>b.onclick=()=>{const plan=b.dataset.plan;if(plan==='free'){history.replaceState(null,'',location.pathname+location.search+'#dashboard');window.dispatchEvent(new HashChangeEvent('hashchange'));return}if(window.__SCHOLARK_BILLING__?.choose)window.__SCHOLARK_BILLING__.choose(plan);else{let x=$('#v115-billing-status');if(!x){x=document.createElement('div');x.id='v115-billing-status';x.style.cssText='margin:14px 0;padding:12px 14px;border-radius:14px;background:#fff0ee;color:#8b352f;font:750 10px/1.45 Inter';p.prepend(x)}x.textContent='Secure payments are not ready yet. Your plan has not changed.'}});}
