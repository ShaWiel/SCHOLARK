(() => {
  ...
  const PLAN_KEY='scholark_v51_planner',GOAL_KEY='scholark_v51_goals',eduKey='scholark_v52_mastery';
  ...
  function renderEducationModule(type){
    ...
    if(type==='curriculum'){
      ...
      box.innerHTML=`<div class="v52-form"><h3>Exam Prep Center</h3><p>Turn a real exam into AI-generated questions and revision priorities, then grade yourself so the results feed Mastery.</p><div class="v52-row"><input id="v52-exam-name" placeholder="Exam / test name"><input id="v52-exam-date" type="date"></div><textarea id="v52-exam-topics" placeholder="Topics or chapters, one per line"></textarea><button class="v52-btn" id="v52-exam-build">Build exam prep with SCHOLARK AI</button><div class="v52-list" id="v52-exam-out"></div></div>`;
    } else if(type==='methods'){
      const methods={
        recall:['Active Recall','Close the notes and retrieve from memory before checking.',['Write 3–5 questions from the topic.','Answer without looking at notes.','Check only after answering.','Mark gaps and retest them later.']],
        feynman:['Feynman Technique','Explain the idea simply, find gaps, then rebuild it.',['Choose one concept.','Explain it as if teaching a beginner.','Circle jargon or gaps you cannot explain.','Return to the source, fix gaps and explain again.']],
        blurting:['Blurting','Write everything you remember, then target what is missing.',['Study briefly, then close the source.','Write everything you can recall.','Compare with the source.','Turn missing points into review questions.']],
        interleave:['Interleaving','Mix related problem types so you practise choosing methods.',['Choose 2–4 related topics.','Mix question types instead of blocking one type.','Name the method before solving.','Review why each method was chosen.']],
        dual:['Dual Coding','Pair concise words with a meaningful visual representation.',['Reduce the topic to key ideas.','Create a diagram, timeline, table or concept map.','Connect each visual element to a short explanation.','Recall the topic using only the visual.']],
        spaced:['Spaced Repetition','Revisit material after increasing gaps.',['Review today.','Review weak points tomorrow.','Review again after 3–4 days.','Increase the gap only when retrieval is strong.']],
        pomodoro:['Pomodoro','Use a focused work block and a real break.',['Choose one concrete task.','Work without switching tasks for 25 minutes.','Take a 5 minute break.','After four cycles, take a longer break.']],
        cornell:['Cornell Notes','Turn notes into a self-testing page.',['Take concise notes in the main column.','Write cue questions beside them.','Add a short summary at the bottom.','Cover the notes and answer the cue questions.']],
        sq3r:['SQ3R','Survey, Question, Read, Recite and Review.',['Survey headings and summaries.','Turn headings into questions.','Read to answer those questions.','Recite answers without looking.','Review the questions later.']],
        leitner:['Leitner System','Move flashcards based on recall strength.',['Create concise question-answer cards.','Start every card in Box 1.','Move correct cards to a later box.','Return missed cards to an earlier box.']]
      };
      box.innerHTML=`<div class="v52-form"><h3>Study Methods Lab</h3><p>Choose a method, apply it to a real topic and add the session directly to Planner.</p><input id="v52-method-topic" placeholder="Topic you want to study"><div class="v52-chiprow">${Object.entries(methods).map(([k,v])=>`<button class="v52-chip" data-method="${k}">${v[0]}</button>`).join('')}</div><div class="v52-list" id="v52-method-out"><div class="v52-item">Choose a method to get an actionable protocol.</div></div></div>`;
      $$('[data-method]',box).forEach(b=>b.onclick=()=>{const m=methods[b.dataset.method];$$('[data-method]',box).forEach(x=>x.classList.toggle('active',x===b));$('#v52-method-out').innerHTML=`<div class="v52-item"><b>${m[0]}</b><br>${m[1]}<div class="v52-method-steps">${m[2].map((x,i)=>`<div class="v52-method-step">${i+1}. ${esc(x)}</div>`).join('')}</div><button class="v52-btn" id="v52-method-plan" style="margin-top:10px;float:none">Add this study session to Planner</button></div>`;$('#v52-method-plan').onclick=()=>{const topic=$('#v52-method-topic').value.trim()||'Study session';addPlan({text:m[0]+' · '+topic,type:'study',subject:topic,date:today(),duration:25,priority:'medium'});$('#v52-method-plan').textContent='✓ Added to Planner'}});
    }
  }
