(() => {
  ...
  function build(){
    ...
    document.body.appendChild(root);$('#v50-go').onclick=search;$('#v50-location-btn').onclick=useLocation;$('.v50-x',root).onclick=close;$('#v50-level').addEventListener('change',syncStudyField);syncStudyField();$('#v50-results').addEventListener('click',e=>{const review=e.target.closest?.('[data-v50-review]');if(review){e.preventDefault();return loadReviews(+review.dataset.v50Review)}if(e.target.closest?.('#v50-more')){visibleLimit=Math.min(renderedItems.length,visibleLimit+PAGE_SIZE);paintResults()}});
  }
  ...
  async function search(){
    const country=$('#v50-country').value.trim(),city=$('#v50-city').value.trim(),level=$('#v50-level').value,study=STUDY_FIELD_LEVELS.has($('#v50-level').value)?$('#v50-study').value.trim():'',radius=Math.min(700,Math.max(1,+$('#v50-radius').value||50)),sort=$('#v50-sort').value,h=$('#v50-results'),loc=$('#v50-location');
