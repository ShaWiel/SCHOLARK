(() => {
  ...
  async function dbSchools(country,city,level,study,pos){
    ...
    try{
      const r=await req('/rest/v1/rpc/search_schools',{method:'POST',body:JSON.stringify({p_country:country,p_city:city||null,p_level:level||'all',p_study:study||null,p_limit:100})});
  ...
  async function search(){
    ...
    let pos=currentPos;
    try{if(!pos||city){const g=await geocode(country,city);pos={lat:g.lat,lon:g.lon};loc.textContent=`Searching around ${g.display}.`}else loc.textContent=`Searching around your current location in ${country}.`;const [db,raw,wiki]=await Promise.all([dbSchools(country,city,level,study,pos),overpass(pos,radius),wikiGeo(pos,radius)]);const local=raw.map(e=>fromOsm(e,pos)).filter(Boolean),wikiItems=wiki.map(x=>({...x,level:'school'})),items=merge(db,local,wikiItems);render(items,{country,city,level,study,radius,sort});loc.textContent+=(db.length?' · '+db.length+' database matches verified before live-source enrichment.':' · Live public sources used; no matching curated database records yet.')}
