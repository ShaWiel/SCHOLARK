(() => {
  ...
  async function search(){
    const country=$('#v50-country').value.trim(),city=$('#v50-city').value.trim(),nameQuery=$('#v50-name')?.value.trim()||'',level=$('#v50-level').value,study=STUDY_FIELD_LEVELS.has($('#v50-level').value)?$('#v50-study').value.trim():'',radius=Math.min(700,Math.max(1,+$('#v50-radius').value||50)),sort=$('#v50-sort').value,h=$('#v50-results'),loc=$('#v50-location');
    ...
    try{
      ...
      render(items,{country,city,nameQuery,level,study,radius:national?700:radius,sort,national});
