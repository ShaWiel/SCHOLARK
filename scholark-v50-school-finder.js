(() => {
  ...
  function comparePanel(){
    ...
    panel.innerHTML='<strong>'+esc(t.compareTitle)+'</strong>'+(selected.length<2?'<div class="v50-state">'+esc(t.noCompare)+'</div>':'<div class="v50-compare-grid">'+selected.map(x=>'<div class="v50-compare-card"><h4>'+esc(x.name)+'</h4><p><b>'+esc(levelLabel[x.level]||'Education')+'</b></p><p>'+esc(x.description||'')+'</p><p>'+esc(x.source||'')+'</p>'+(isVerified(x)?'<span class="v50-tag v50-verified">'+esc(t.verified)+'</span>':'')+metricMarkup(x)+(x.phone?'<p>'+esc(x.phone)+'</p>':'')+'</div>').join('')+'</div>');
  }
