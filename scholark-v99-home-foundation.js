(() => {
  ...
  function repair(){
    ...
    const previewHealthy=window.__SCHOLARK_V32_PREVIEW__?.healthy?.();
  ...
  function health(){
    ...
    const report={
      ...
      topbarLanguage:!home()||!!$('#v55-language')&&$('#v55-language').options.length===7&&getComputedStyle($('#v55-language')).display!=='none',
    ...
    report.ok=!report.home||(report.activeMode===report.previewMode&&report.previewComplete&&report.stageReady&&report.promptReady&&report.promptMultiline&&report.topbarLanguage&&report.presenterReady&&report.masteryBars===3&&report.masteryAnimated&&report.runtimeErrors.length===0);
    try{sessionStorage.setItem('scholark_home_foundation_r120',JSON.stringify(report))}catch{}
    console[report.ok?'log':'warn']('[SCHOLARK] Home foundation R119 '+(report.ok?'PASS':'WARN'),report);
  ...
  window.__SCHOLARK_HOME_FOUNDATION__={repair:schedule,health,release:'r120'};
})();
