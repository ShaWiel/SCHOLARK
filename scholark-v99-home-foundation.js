(() => {
  ...
  function patchTranslationCaches(){
    for(const [lc] of UI_LANGUAGE_OPTIONS){
      if(lc==='en')continue;
      const key='scholark_v90_i18n_v8-global74-resilient_'+lc;let saved={};try{saved=JSON.parse(localStorage.getItem(key)||'{}')||{}}catch{}
  ...
  function health(){
    ...
    const report={
      ok:true,release:'r194',home:home(),route:route(),activeMode:mode,previewMode,stageMode,statusModes,
  ...
  addEventListener('scholark-home-mode-change',()=>setTimeout(repairLocalization,30));addEventListener('scholark-language-applied',()=>setTimeout(repairLocalization,30));addEventListener('scholark-language-ready',()=>setTimeout(repairLocalization,100));
  ...
  [100,600].forEach(ms=>setTimeout(()=>schedule(0),ms));setTimeout(health,2400);
  ...
  window.__SCHOLARK_HOME_FOUNDATION__={repair:schedule,health,repairLocalization,repairProject,repairStudio,prewarmStudio,release:'r194',documentWideObserver:false};
})();
