(() => {
  ...
  async function process(files){
    ...
    try{
      ...
      state.text=state.text.slice(0,80000);renderFiles();status(state.files.filter(x=>x.text).length+' readable file(s) ready for SCHOLARK AI.');enableActions();
  ...
  async function ai(mode,payload){
    ...
    try{const r=await fetch('/api/learning/generate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({mode,level:localStorage.getItem('scholark_learning_level')||'student',language:localStorage.getItem('scholark_ui_language')||'English',...payload}),signal:ctrl.signal});const d=await r.json().catch(()=>({}));if(!r.ok||!d?.ok)throw new Error(d?.error||'AI request failed');return d}finally{clearTimeout(timer)}
  }
  ...
  async function action(kind,btn){
    ...
    try{
      ...
      if(kind==='quiz'){data=await ai('exam',{prompt:'Create a rigorous practice quiz based only on the uploaded material.',subject:'Uploaded material',topics:['Uploaded material'],count:12,difficulty:'mixed',context});title='Practice quiz';const r=data.result;out.innerHTML='<h3>'+esc(r.title||title)+'</h3>'+esc(r.instructions||'')+'\n\n'+(r.questions||[]).map((q,i)=>(i+1)+'. '+q.prompt+'\nAnswer: '+q.answer+'\nWhy: '+q.explanation).map(esc).join('\n\n')+'<div class="v86-meta">'+esc(data.provider||'AI')+' · '+esc(data.model||'')+'</div>';return}
      const prompts={
      ...
      data=await ai('tutor',{prompt:prompts[kind],context,tutorMode:'teach'});title={summary:'Summary',explain:'Explanation',flashcards:'Flashcards',notes:'Study notes',plan:'Study plan'}[kind]||'Result';
