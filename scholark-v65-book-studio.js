(() => {
  ...
  function legacyTestBook(book){
    if(!book)return false;
    if(/scholark-test-engine/i.test(String(book.provider||''))||/local-(?:book|manuscript)/i.test(String(book.model||'')))return true;
    return Object.values(book.drafts||{}).some(d=>(d?.sections||[]).some(s=>/in this part of|the scene moves through|the characters respond|the genre tone|the pov perspective|testing-mode draft prose|replace this draft/i.test(String(s?.body||''))));
  }
  function load(){
    try{state.book=JSON.parse(localStorage.getItem(STORE)||'null')}catch{state.book=null}
    if(legacyTestBook(state.book)){
      state.book={...state.book,plan:null,drafts:{},provider:'',model:'',migratedR113:true,updated:Date.now()};
      try{localStorage.setItem(STORE,JSON.stringify(state.book))}catch{}
    }
  }
  function save(){
  ...
  function shell(){
    ...
    Array.from(document.querySelectorAll('#v51-sidebar [data-v51-tool]')).forEach(b=>b.classList.toggle('active',b.dataset.v51Tool==='book'));
    ...
    Array.from(main.querySelectorAll('.v51-page')).forEach(p=>{p.classList.remove('active');p.style.display='none'});
