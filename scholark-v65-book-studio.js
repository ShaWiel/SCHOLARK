(() => {
  ...
  function values(){return {name:clean($('#v65-name')?.value),genre:clean($('#v65-genre')?.value),concept:clean($('#v65-concept')?.value),audience:clean($('#v65-audience')?.value),pov:$('#v65-pov')?.value||'third',targetWords:+($('#v65-words')?.value||80000),chapterCount:Math.max(3,Math.min(40,+($('#v65-count')?.value||12)))}}
  function planPrompt(v){return 'Create a complete '+v.chapterCount+'-chapter book blueprint for: '+v.concept+'. Genre/type: '+(v.genre||'choose the best fit')+'. Audience: '+(v.audience||'infer it')+'. POV: '+v.pov+'. Target length: about '+v.targetWords+' words. Make every chapter specific, progressive and continuity-aware.'}
  ...
  function exportBook(){
    syncDraft(); if(!state.book)return; const b=state.book,chs=chapters(); let body='<h1>'+esc(b.name)+'</h1><p>'+esc(b.plan?.summary||b.concept)+'</p>';
    chs.forEach((ch,i)=>{const d=b.drafts?.[i];body+='<h2>Chapter '+(i+1)+': '+esc(ch.title)+'</h2>';if(d)body+=(d.sections||[]).map(s=>'<h3>'+esc(s.title||'')+'</h3><p>'+esc(s.body||'').replace(/\n/g,'<br>')+'</p>').join('');else body+='<p><em>Chapter plan:</em> '+esc(ch.body||ch.subtitle||'')+'</p>'});
    const html='<!doctype html><meta charset="utf-8"><style>body{max-width:760px;margin:50px auto;font-family:Georgia,serif;line-height:1.7}h2{page-break-before:always}</style>'+body,blob=new Blob([html],{type:'application/msword'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=(b.name||'scholark-book').replace(/[^a-z0-9-_]+/gi,'-').toLowerCase()+'.doc';a.click();setTimeout(()=>URL.revokeObjectURL(u),500);
  }
