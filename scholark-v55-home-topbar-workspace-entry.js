(() => {
  ...
  const workspace=()=>/dashboard|studio|tutor|education|language|planner|progress|goal|project|files|schools|study|book|presentation|webpage|document|report|graphic|social/.test(String(location.hash||'').toLowerCase());
  const publicHome=()=>!workspace()&&!/login|signin|account/.test(String(location.hash||'').toLowerCase());
