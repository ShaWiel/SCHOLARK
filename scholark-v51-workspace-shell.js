(() => {
  ...
  const TOOLS=[
    ['dashboard','⌂','Dashboard'],['ai','✺','ARKI'],['tutor','AI','AI Tutor'],['education','◎','Education & Learning'],['language','Aa','Language Learner'],['planner','▦','Planner'],['focus','◷','Focus Sessions'],['flashcards','▤','Flashcards'],['assignments','✓','Assignments'],['progress','↗','Progress'],['goal','◉','Goals'],['files','▣','Files & Notes'],['project','▧','My Projects']
  ];
  const PRO=[['schools','⌖','Schools Near Me','PRO'],['study','🚀','Study Ahead','PRO']];
  const COMING=[['studio','✦','Studio AI'],['book','📚','Book Studio']];
  ...
  style.textContent=`
    ...
    html.v51-workspace-root,body.v51-workspace{overflow:hidden!important;overscroll-behavior:none!important}
  ...
  function showFallback(id){clearModes();setRoute(id);state.active=id;syncNav();showPage('fallback');const host=$('#v51-fallback');const title={ai:'ARKI',tutor:'AI Tutor',education:'Education & Learning',planner:'Planner',progress:'Progress',goal:'Goals',project:'My Projects'}[id]||id;let body='';
    ...
    host.innerHTML=`<div class="v51-fallback"><div class="v51-fallback-card"><div class="v51-head"><div><small>SCHOLARK WORKSPACE</small><h2>${esc(title)}</h2><p>Workspace rescue view. SCHOLARK no longer sends you back to Dashboard when a legacy view is unavailable.</p></div></div>${body}</div></div>`;wireFallback(id)
  }
