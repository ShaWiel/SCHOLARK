(() => {
  ...
  const workspaceRoute=()=>/dashboard|studio|tutor|education|language|planner|focus|flashcards|assignments|progress|goal|project|files|schools|study|book|presentation|webpage|document|report|graphic|social/.test(route());
  ...
  const ALIASES={
    tutor:['ai tutor','tutor ai','tutor'],education:['education & learning','educatie & leren','onderwijs & leren','education'],language:['language learner','language learning','talen leren','taal leren'],planner:['planner'],focus:['focus sessions','focus','concentratie'],flashcards:['flashcards','flash cards','kaartjes'],assignments:['assignments','assignment','opdrachten'],progress:['progress','voortgang'],goal:['goals','doelen','goal'],project:['my projects','mijn projecten','projects','projecten']
  };
  ...
  function renderLevels(){
    ...
    if(suriname){
      host.innerHTML=SURINAME_GROUPS.map(group=>{
        ...
        return `<section class="v51-level-cluster ${group.tone}" data-v51-group="${group.id}" data-v96-i18n-owned="1"><div class="v51-level-group">${esc(group.label)}</div><div class="v51-level-cluster-cards">${cards}</div></section>`;
      }).join('');
