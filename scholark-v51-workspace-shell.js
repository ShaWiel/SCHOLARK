(() => {
  ...
  function applyLanguageRoot(el){
    ...
    // Workspace roots are reused between tools. Never cache translation state by
    // root element, because the same #v51-main / fallback can receive new UI.
    i18n.apply(el);
  }
  function syncWorkspaceLanguage(root=null,force=false){
    ...
    // v41-language is an OUTPUT-language selector and intentionally supports
    // more languages than the 7-language SCHOLARK interface.
    ...
    if(lang!=='en'){
      ...
      setTimeout(()=>{if(workspaceRoute()&&(localStorage.getItem('scholark_ui_language')||'nl')===lang){i18n?.syncSelectors?.();i18n?.translateCurrentPage?.(false)}},320);
    }
  }
