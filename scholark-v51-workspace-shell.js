(() => {
  ...
  const TOOLS=[
    ['dashboard','⌂','Dashboard'],['studio','✦','Studio AI'],['ai','✺','ARKI'],['tutor','AI','AI Tutor'],['education','◎','Education & Learning'],['language','Aa','Language Learner'],['planner','▦','Planner'],['focus','◷','Focus Sessions'],['flashcards','▤','Flashcards'],['assignments','✓','Assignments'],['progress','↗','Progress'],['goal','◉','Goals'],['files','▣','Files & Notes'],['project','▧','My Projects']
  ];
  const PRO=[['schools','⌖','Schools Near Me','PRO'],['study','🚀','Study Ahead','PRO'],['book','📚','Book Studio','COMING SOON']];
  const INACTIVE=new Set(['studio','book']);
  const ALIASES={
  ...
  function build(){
    if(side)return;
    side=document.createElement('aside');side.id='v51-sidebar';side.innerHTML=`<div class="v51-brand"><div class="v51-logo"></div><div class="v51-brand-copy"><b>SCHOLARK</b><span>WORKSPACE</span></div></div><div class="v51-section">WORKSPACE</div>${TOOLS.map(([id,ic,l])=>`<button class="v51-nav ${INACTIVE.has(id)?'v51-inactive':''}" data-v51-tool="${id}" ${INACTIVE.has(id)?'data-v51-inactive="1" aria-disabled="true"':''}><i>${ic}</i><span>${l}</span>${INACTIVE.has(id)?'<em>COMING SOON</em>':''}</button>`).join('')}<div class="v51-section">FUTURE & PRO</div>${PRO.map(([id,ic,l,b])=>`<button class="v51-nav ${INACTIVE.has(id)?'v51-inactive':''}" data-v51-tool="${id}" ${INACTIVE.has(id)?'data-v51-inactive="1" aria-disabled="true"':''}><i>${ic}</i><span>${l}</span><em>${b}</em></button>`).join('')}<div class="v51-quality"><b>AI QUALITY · MAX</b><span>Highest available quality, expert depth, research, source checking and final polish.</span></div>`;document.body.appendChild(side);
  ...
  function card(id,ic,title,desc,primary=false){const inactive=INACTIVE.has(id);return `<button class="v51-card ${primary?'primary':''} ${inactive?'inactive':''}" data-v51-tool="${id}" ${inactive?'data-v51-inactive="1" aria-disabled="true"':''}><span class="icon">${ic}</span><h3>${title}</h3><p>${desc}</p><b>${inactive?'COMING SOON':'OPEN '+title.toUpperCase()+' →'}</b></button>`}
  ...
  function showComingSoon(id){
    setCollapsed(false,true);clearModes();state.active=id;syncNav();setRoute(id);showPage('fallback');
    const host=$('#v51-fallback');if(!host)return;
    const isBook=id==='book',title=isBook?'Book Studio':'Studio AI';
    host.innerHTML=`<div class="v51-coming-soon"><div class="v51-coming-card"><span class="v51-coming-badge">COMING SOON</span><h1>${title}</h1><p>${isBook?'Book Studio is temporarily inactive while we rebuild long-form generation to a higher quality and reliability standard.':'Studio AI is temporarily inactive while we rebuild creation quality beyond what the current free-AI stack can deliver reliably.'}</p><p>The rest of SCHOLARK remains available. Your existing workspace data is kept, and this tool will return when it meets the quality standard again.</p><div class="v51-coming-actions"><button type="button" data-v51-coming="ai">Open ARKI</button><button type="button" class="alt" data-v51-coming="dashboard">Back to Dashboard</button></div></div></div>`;
